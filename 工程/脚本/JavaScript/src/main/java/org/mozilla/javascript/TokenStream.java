...
class TokenStream
{
    ...
    private static int stringToKeyword(String name, int version, boolean isStrict)
    {
        ...
        } else //( version < Context.VERSION_CN) {
            return stringToKeywordForES(name,isStrict);
        /*
            } else {
            return stringToKeywordForCN(name, isStrict);
            }
        */
    ...
    private static int stringToKeywordForCN(String name) {
        switch(name.length()) {
            case 1:
                switch(name) {
                    case "真":return Token.TRUE;
                   case "假":return Token.FALSE;
                   case "空":return Token.NULL;
                   case "当":return Token.WHILE;
                   case "和":return Token.WITH;
                }
             case 2:
                 switch(name) {
                     case "如果":return Token.IF;
                     case "否则":return Token.ELSE;
                     case "返回":return Token.RETURN;
                 }
        }
    }
    */
