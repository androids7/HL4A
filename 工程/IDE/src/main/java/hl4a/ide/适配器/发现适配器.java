...
public class 发现适配器 extends 基本适配器 {
    ...
    public void 更新() {
        ...
        for (Map.Entry $单个 : $列表.entrySet()) {
            ...
            String $地址 = ((String)$单个.getValue()).replace("<中心>", 更新.中心);
            哈希表 $表 = new 哈希表();
            $表.设置("包名", $名称);
            $表.设置("地址", $地址);
            数据.添加($表);
    ...
    public View 处理(View $视图,哈希表<String, Object> $参数) {
        ...
        $布局.名称.置文本($包名);
        $布局.图标.setImageResource(android.R.drawable.sym_def_app_icon);
    ...
    public void 加载内容(布局_适配器_应用 $视图,String $包名,String $地址) {
        ...
        if (文件.是文件($缓存 + "/图标.icon")) {
            处理.主线程($视图.图标, "置图片", 图片.读取($缓存 + "/图片.icon"));
        } else {
