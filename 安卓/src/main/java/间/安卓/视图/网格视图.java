...
public class 网格视图 extends GridView implements 基本列表 {
    ...
    public 网格视图(ViewGroup $父视图) {
        this($父视图.getContext());
        加入到($父视图);
    }
    
    public void 置行边距(Object $边距) {
        setVerticalSpacing(视图.检查大小($边距));
    }
    
    public void 置列边距(Object $边距) {
        setHorizontalSpacing(视图.检查大小($边距));
    }
    
    public void 置行视图数(int $行数) {
        setNumColumns($行数);
    }
    
    public void 置每列宽度(Object $宽度) {
        setColumnWidth(视图.检查大小($宽度));
    }
