    ...
    public synchronized void 推送(哈希表 $列表) {
        ...
        for (Map.Entry $单个 : $列表.entrySet()) {
            ...
            if (!文件.是目录($检查)) {
                ...
                if (更新.设置 != null)
                    $地址 = $地址.replace("<中心>", 更新.设置.中心);
                new 线程(this, "检查更新").启动($包名, $地址);
            }
        }
    }

    public void 检查更新(String $包名,String $地址) {
        if (是断网) return;
        String $缓存 = 应用配置信息.应用保存 + "/" + $包名;
        应用信息 $保存 = 解析($缓存, "应用.yml");
        资源 $返回 = 连接.创建($地址 + "/应用.yml", "GET").同步();
        if ($返回.已成功()) {
            String $文本 = $返回.文本();
            应用信息 $信息 = YAML.解析($文本, 应用信息.class);
            if ($信息.版本号 > $保存.版本号) {
                字符.保存($缓存 + "/应用.yml", $文本);
                资源 $图标 = 连接.创建($地址 + "/图标.png", "GET").同步();
                if ($图标.已成功()) {
                    byte[] $字节 = $图标.字节();
                    字节.保存($缓存 + "/图标.png", $字节);
                }
                ...
                String $下载 = 文件.取缓存目录($包名 + ".zip");
                $内容.保存($下载);
                ZIP.解压($下载, $缓存);
                文件.删除($下载);
                文件.剪切($缓存, 应用配置信息.应用保存 + "/" + $下载);
            }
        }
    }
